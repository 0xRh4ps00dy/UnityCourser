...
def page_id(index: int) -> str:
    return f"page-{index:02d}-{uuid.uuid4().hex[:8]}"
...
def nav_html(items: list[dict], active_index: int) -> str:
    ...
    for i, item in enumerate(items, start=1):
        ...
        if active_index == i:
            lines.append(f'<li class="active"> <a href="{file_name}" class="active no-ch">{title}</a>')
        else:
            lines.append(f'<li> <a href="{file_name}" class="no-ch">{title}</a>')
        lines.append("</li>")
...
def page_template(*, package_title: str, package_subtitle: str, page_title: str, nav_block: str, content_html: str, prev_href: str | None, next_href: str | None, is_index: bool, page_anchor: str, base_prefix: str) -> str:
    prev_button = (
...
def build_content_xml(items: list[dict], package_title: str, package_subtitle: str, source_base: Path) -> str:
    ...
    lines = [
        ...
        '<odeNavStructures>',
        '  <odeNavStructure>',
        '    <odePageId>index-node</odePageId>',
        ...
        '      <odePagStructure>',
        '        <odePageId>index-node</odePageId>',
        '        <odeBlockId>index-node-blk-01</odeBlockId>',
        ...
        '            <odePageId>index-node</odePageId>',
        '            <odeBlockId>index-node-blk-01</odeBlockId>',
        '            <odeIdeviceId>index-node-idev-01</odeIdeviceId>',
        '            <odeIdeviceTypeName>text</odeIdeviceTypeName>',
        f'            <htmlView><![CDATA[{escape_cdata(build_index_sections(source_base, len(items)))}]]></htmlView>',
        f'            <jsonProperties><![CDATA[{escape_cdata(json.dumps({"ideviceId": "index-node-idev-01", "textTextarea": build_index_sections(source_base, len(items)), "textFeedbackInput": "", "textFeedbackTextarea": ""}, ensure_ascii=False))}]]></jsonProperties>',
    ...
    for i, item in enumerate(items, start=1):
        ...
        component_html = build_item_sections(item, i, source_name)
        component_id = f"unit-{i:02d}-idev-01"
        json_props = {
            "ideviceId": component_id,
            "textTextarea": component_html,
            "textFeedbackInput": "",
        ...
        lines.extend(
            [
                '  <odeNavStructure>',
                f'    <odePageId>unit-{i:02d}</odePageId>',
                '    <odeParentPageId></odeParentPageId>',
                ...
                '      <odePagStructure>',
                f'        <odePageId>unit-{i:02d}</odePageId>',
                f'        <odeBlockId>unit-{i:02d}-blk-01</odeBlockId>',
                '        <blockName>Contenido</blockName>',
                '        <iconName></iconName>',
                '        <odePagStructureOrder>0</odePagStructureOrder>',
                '        <odePagStructureProperties>',
...
def main() -> int:
    ...
    for i, item in enumerate(items, start=1):
        ...
        prev_href = "../index.html" if i == 1 else f"{page_filename(i - 1)}"
        next_href = None if i == len(items) else f"{page_filename(i + 1)}"
        ...
        page_html = page_template(
            ...
            next_href=next_href,
            is_index=False,
            page_anchor=page_id(i),
            base_prefix="../",
        )
        (html_dir / page_filename(i)).write_text(page_html, encoding="utf-8")

    content_xml = build_content_xml(items, package_title, package_subtitle, source_base)
